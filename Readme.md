...
> ⚠️ **Переход к MVP.** Мы готовим урезанную версию, которая оставляет только склад, заказы и ремонт. Перед началом работ изучите чек-лист зависимостей и рисков в документе [`docs/mvp-cutover-risks.md`](docs/mvp-cutover-risks.md).

## Быстрый запуск (Docker)

1. Скопируйте файл окружения и отредактируйте значения при необходимости:
   ```bash
   cp .env.example .env
   ```
2. Поднимите совместимую базу данных (например, MySQL/MariaDB):
   ```bash
   docker run --name lamerms-db -e MYSQL_DATABASE=adam_rms \
     -e MYSQL_USER=adam_rms -e MYSQL_PASSWORD=secret -e MYSQL_ROOT_PASSWORD=secret \
     -p 3307:3306 -d mariadb:10.11
   ```
3. Соберите образ приложения и запустите контейнер, указав `.env`:
   ```bash
   docker build -t lamerms .
   docker run --name lamerms-app --env-file .env --link lamerms-db:db \
     -p 8080:80 lamerms
   ```
4. Если сиды ещё не адаптированы под MVP, установите в `.env` значение `SEED_ON_START=false`, чтобы пропустить их на старте.
